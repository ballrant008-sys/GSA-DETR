...
def _rotate_covariance_2x2(Sigma, theta):
    theta = theta.squeeze(-1) if theta.ndim == Sigma.ndim - 1 else theta
    cos_t = torch.cos(theta)
    sin_t = torch.sin(theta)

    r1 = torch.stack([cos_t, -sin_t], dim=-1)
    r2 = torch.stack([sin_t,  cos_t], dim=-1)
    R = torch.stack([r1, r2], dim=-2)

    return R @ Sigma @ R.transpose(-1, -2)


def bbox_agriou(box1, box2, theta_max=1.0, C=1.0, eps=1e-7):
    if box1.shape[-1] not in (4, 5):
        raise ValueError(f"bbox_agriou expects last dim=4 or 5 for box1, got {box1.shape[-1]}")
    if box2.shape[-1] not in (4, 5):
        raise ValueError(f"bbox_agriou expects last dim=4 or 5 for box2, got {box2.shape[-1]}")

    cx1, cy1, w1, h1 = box1[..., :4].chunk(4, dim=-1)
    cx2, cy2, w2, h2 = box2[..., :4].chunk(4, dim=-1)

    zero1 = torch.zeros_like(cx1)
    zero2 = torch.zeros_like(cx2)

    mu1, Sigma1 = _gaussian_from_params(cx1, cy1, w1, h1, zero1, eps=eps)
    mu2, Sigma2 = _gaussian_from_params(cx2, cy2, w2, h2, zero2, eps=eps)

    a = (h1 - w1).abs() / (h1 + w1 + eps)
    theta_a = theta_max * a

    candidates = torch.cat(
        [
            torch.zeros_like(theta_a),
            theta_a,
            -theta_a,
        ],
        dim=-1,
    ...
    d_mu = mu1 - mu2
    term_mu = (d_mu ** 2).sum(dim=-1)

    S1_sqrt = _sqrtm_2x2(Sigma1, eps=eps)

    distances = []
    for i in range(candidates.shape[-1]):
        theta = candidates[..., i:i + 1]
        Sigma2_rot = _rotate_covariance_2x2(Sigma2, theta)
        S2_sqrt = _sqrtm_2x2(Sigma2_rot, eps=eps)

        d_S = S1_sqrt - S2_sqrt
        term_S = (d_S ** 2).sum(dim=(-1, -2))

        distances.append(term_mu + term_S)

    D_agr = torch.stack(distances, dim=-1).min(dim=-1).values
    agr_iou = 1.0 / (1.0 + D_agr / (float(C) + eps))

    return agr_iou.unsqueeze(-1)
...
class DETRLoss(nn.Module):
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=''):
        ...
        if self.use_wiseiou:
            ...
            loss[name_giou] = 1.0 - bbox_agriou(pred_bboxes, gt_bboxes, C=self.rgm_C)
        else:
    ...
    def _get_loss(self,
        ...
        if len(gt_bboxes):
            if getattr(self, 'rgm_loss', False):
                gt_scores[idx] = bbox_agriou(pred_bboxes.detach(), gt_bboxes, C=self.rgm_C).squeeze(-1)
            else:
